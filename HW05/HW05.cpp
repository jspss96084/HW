...
int main (void){
    ...
    # pragma omp parallel num_threads( NThread )      // set the number of threads
    # pragma omp parallel       // add the "parallel" construct
    ...
    // Derived constants
    double dn = L / (N - 1); // spatial resolution
    ...
    SOR_method(u, u_in, rho, dn);
    err = calculateError(u, u_ref);
    printf("num_threads: %d", NThread);
    /*
    for (int i = 0; i < ROW; i++){
        for (int j = 0; j < COL; j++){
            printf(" %f ", u[i][j]);
    ...
    */
    ...
    save_M(u_ref, "./SOR_reference.txt");
...
void SOR_method(double **u, double **u_in, double **rho, double dx){
    ...
    for (int count=0; count<iter_max; count++) {
        ...
        // Update all cells
        # pragma omp parallel for reduction ( +:err )      // add the "for" construct
        for (int i = 1; i < N - 1; ++i) {
            for (int j = 1; j < N - 1; ++j) {
                int mod = (i+j)%2;
                if (mod == 0){
...
void init_rho(double **rho, double dx, double dy) {
    ...
    for (int i=0; i<ROW; i++){
        for (int j=0; j<COL; j++){
            rho[i][j] = -(2*pow(pi,2))*sin(pi*i*dx)*cos(pi*j*dy);
        }
    }
}
