...
def Crank_Nicolson(N):
   ...
   N   = N   # number of equally spaced sampling points
   ...
   # set the coefficient matrices A & B with A*u(t+dt)=B*u(t)
   r = D*dt/dx**2
#   A = np.diagflat( np.ones(N-3)*(-r),       -1 ) + \
#      np.diagflat( np.ones(N-2)*(1.0+2.0*r), 0 ) + \
#      np.diagflat( np.ones(N-3)*(-r),       +1 );
   A = np.diagflat( np.ones(N-3)*(-0.5*r), -1) + \
      np.diagflat( np.ones(N-2)*(1+r),  0) + \
      np.diagflat( np.ones(N-3)*(-0.5*r), +1);
   B = np.diagflat( np.ones(N-3)*(0.5*r), -1) + \
      np.diagflat( np.ones(N-2)*(1-r),  0) + \
      np.diagflat( np.ones(N-3)*(0.5*r), +1);
